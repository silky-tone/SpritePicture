...
function handlerStyle(item: any) {
  return `.${props.name}.${props.name} {width: ${handlerSize(item.width)}; height: ${handlerSize(item.height)}; perspective-origin: ${handlerSize(-item.left)} ${handlerSize(-item.top)};}`;
}
